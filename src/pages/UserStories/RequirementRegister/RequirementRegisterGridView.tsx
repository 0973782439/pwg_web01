...
const RequirementRegisterGridView: React.FC<Props> = ({ itemRender, columnItemFolderGrid, stateRequirementGridChart, handleRowClick, rowClassName }) => {
    ...
    const searchArray = (array: IUserStories, searchTerm: string) => {
        ...
        return array.items.filter((item: ItemFolderGrid) => {
            ...
            return title.includes(searchLower) || assigneeName.includes(searchLower) || businessImportance.includes(searchLower) || folderNameShow.includes(searchLower);
        });

    }
    const handleSearchRequirement = (e: React.ChangeEvent<HTMLInputElement>) => {
